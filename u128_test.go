...
func TestU128AsBigInt(t *testing.T) {
	for idx, tc := range []struct {
		...
		{U128{hi: 0, lo: 2}, bigU64(2)},
		{U128{hi: 0xFFFFFFFFFFFFFFFF, lo: 0xFFFFFFFFFFFFFFFE}, bigs("0xFFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFE")},
		{U128{hi: 0x1, lo: 0x0}, bigs("18446744073709551616")},
		{U128{hi: 0x1, lo: 0xFFFFFFFFFFFFFFFF}, bigs("36893488147419103231")}, // (1<<65) - 1
		{U128{hi: 0x1, lo: 0x8AC7230489E7FFFF}, bigs("28446744073709551615")},
		{U128{hi: 0x7FFFFFFFFFFFFFFF, lo: 0xFFFFFFFFFFFFFFFF}, bigs("170141183460469231731687303715884105727")},
		{U128{hi: 0xFFFFFFFFFFFFFFFF, lo: 0xFFFFFFFFFFFFFFFF}, bigs("0x FFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFF")},
		{U128{hi: 0x8000000000000000, lo: 0}, bigs("0x 8000000000000000 0000000000000000")},
	} {
...
func TestU128FromBigInt(t *testing.T) {
	for idx, tc := range []struct {
		...
		{bigs("0x FFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFF"), U128{hi: 0xFFFFFFFFFFFFFFFF, lo: 0xFFFFFFFFFFFFFFFF}, true},
...
func BenchmarkU128Rem64(b *testing.B) {
	b.Run("fast", func(b *testing.B) {
		u, v := U128{hi: 1, lo: 0}, uint64(56) // u.hi < v
		for i := 0; i < b.N; i++ {
	...
	b.Run("slow", func(b *testing.B) {
		u, v := U128{hi: 100, lo: 0}, uint64(56) // u.hi >= v
		for i := 0; i < b.N; i++ {
			BenchU128Result = u.Rem64(v)
		}
	})
